...
public class Main {
    public static void main(String[] args) {
        ...
        int maxDislikes = 0;
        Set<Video> mostDislikedVideos = new HashSet<>();
        ...
        // Additional Functionality: Use of Map to track videos by title
        Map<String, Video> videoMap = new HashMap<>();
        for (Video video : blog.getVideos()) {
            videoMap.put(video.getTitle(), video);
        }

        // Example: Retrieve a video by its title
        String searchTitle = "Java Basics";
        Video searchedVideo = videoMap.get(searchTitle);
        if (searchedVideo != null) {
            System.out.println("Found video: " + searchedVideo.getTitle() + " (URL: " + searchedVideo.getUrl() + ")");
        } else {
            System.out.println("Video with title '" + searchTitle + "' not found.");
        }
    }
}